...
export type WorkflowRepair = { outcome: 'repaired' | 'failed'; sourceState: 'unconfigured' | 'conflict'; errorCode?: string; message?: string };
export type Issue = {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        throw new Error([payload.error, payload.detail].filter(Boolean).join(': ') || text);
      } catch (cause) {
